...
import 'package:sikka/pages/event3payment.dart';
...
//  final String x;
//  Event3({Key key, @required this.x}) : super(key: key);
...
class _Event3State extends State<Event3> {
  ...
  storeuser() async {
    ...
    await Firestore.instance.collection("Event3").document(userid).setData({
      "color": x,
      "userid": userid,
      "coin": 0,
      'time': DateTime.now(),
            ...
            onPressed: () {
              ...
              if (s > 0) {
                 ...
                 Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => Payment3(color: x),
                  ));
              } else {
                _showDialog2();
              }
